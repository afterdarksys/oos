...
func applyOverrides(cfg *config.Config, o *opts) error {
	if o.critGB > 0 {
...
func doFree(cfg *config.Config, o *opts, out, errw io.Writer) int {
...
type whyResult struct {
	...
	Entry    *config.Entry `json:"entry,omitempty"`   // the configured entry that covers it, if any
...
func doWhy(cfg *config.Config, env guard.Env, o *opts, out, errw io.Writer) int {
	p := config.ExpandHome(o.why, env.Home)
	if !filepath.IsAbs(p) {
	...
	var ent *config.Entry
	for _, e := range cfg.Entries(nil) {
		e := e
		switch {
		case e.Path == p:
			ent, r.Covered = &e, "exact"
		case config.IsUnder(p, e.Path) && ent == nil:
			ent, r.Covered = &e, "under-entry"
		case config.IsUnder(e.Path, p) && ent == nil:
	...
	judge := config.Entry{Path: p, Type: "adhoc", Action: config.ActionRmContents}
	if r.Exists && !r.IsDir {
		judge.Action = config.ActionRm
	}
	if ent != nil {
		r.Entry = ent
		if r.Covered == "exact" {
	...
	switch {
	case ent != nil && r.Covered == "exact" && ent.Action == config.ActionNever:
		r.Verdict, r.ExitCode = "never", status.ExitCritical
		r.Reasons = append(r.Reasons, "entry is marked never")
	case ent != nil && r.Covered == "under-entry" && ent.Action == config.ActionNever:
		r.Verdict, r.ExitCode = "never", status.ExitCritical
		r.Reasons = append(r.Reasons, "inside an entry marked never; the parent protects it")
	case ent != nil && r.Covered == "exact" && ent.Action == config.ActionCommand:
		r.Verdict, r.ExitCode = "would-act", status.ExitOK
	...
	default:
		if err := env.CheckDeletable(cfg.Policy, judge); err != nil {
			...
			if ent != nil && r.Covered == "exact" {
				r.Verdict, r.ExitCode = "refused", status.ExitCritical
			} else {
				r.Verdict, r.ExitCode = "unknown-refused", status.ExitCritical
			}
		} else if ent != nil && r.Covered == "exact" {
			r.Verdict, r.ExitCode = "would-act", status.ExitOK
			r.Reasons = append(r.Reasons, "all guards pass for "+ent.Action)
		} else {
			r.Verdict, r.ExitCode = "unknown-allowed", status.ExitWarn
	...
	fmt.Fprintf(out, "%s\n", p)
	if r.Exists {
		fmt.Fprintf(out, "  %s, %s\n", map[bool]string{true: "directory", false: "file"}[r.IsDir], size.Human(r.Bytes))
	} else {
...
func configFileForEdit(o *opts, home string) (string, error) {
	...
	for _, c := range config.Candidates(home) {
...
func rewriteConfig(path, home string, edit func(doc map[string]any) error) error {
	...
	if _, err := config.Parse(outB, home); err != nil {
...
func doAdd(env guard.Env, o *opts, out, errw io.Writer) int {
	path, err := configFileForEdit(o, env.Home)
	if err != nil {
		fmt.Fprintln(errw, "oos:", err)
		return status.ExitUsage
	}
	target := config.ExpandHome(o.add, env.Home)
	...
	stored := target
	if config.IsUnder(target, env.Home) {
	...
	if tags := config.SplitTags(o.addTags); len(tags) > 0 {
		entry["tags"] = tags
	}
...
func doForget(env guard.Env, o *opts, out, errw io.Writer) int {
	path, err := configFileForEdit(o, env.Home)
	if err != nil {
		fmt.Fprintln(errw, "oos:", err)
		return status.ExitUsage
	}
	target := config.ExpandHome(o.forget, env.Home)
	removed := ""
	err = rewriteConfig(path, env.Home, func(doc map[string]any) error {
		for _, list := range []string{"known_dirs", "known_files"} {
			...
			for _, x := range arr {
				...
				if ok {
					if ps, _ := m["path"].(string); config.ExpandHome(ps, env.Home) == target {
						removed = list
						continue
					}
				}
...
func doLogTail(cfg *config.Config, o *opts, out, errw io.Writer) int {
