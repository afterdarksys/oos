...
func gitRepo(t *testing.T, dir string, when time.Time) {
	...
	run("init", "-q")
	testutil.Write(t, filepath.Join(dir, "README"), 100)
	run("add", "README")
	run("commit", "-q", "-m", "init")
}
...
func TestScanBuildsReport(t *testing.T) {
	...
	testutil.Write(t, filepath.Join(idle, "Cargo.toml"), 10)
	testutil.Write(t, filepath.Join(idle, "src", "main.rs"), 200)
	testutil.Write(t, filepath.Join(idle, "target", "debug", "bin"), 4<<20)
	for _, p := range []string{"Cargo.toml", "src/main.rs", "README", "src", ""} {
		testutil.Age(t, filepath.Join(idle, p), 90*24*time.Hour)
	}
	...
	testutil.Write(t, filepath.Join(dirty, "package.json"), 10) // untracked = dirty
	testutil.Write(t, filepath.Join(dirty, "node_modules", "left-pad", "index.js"), 2<<20)
	...
	testutil.Write(t, filepath.Join(dev, "not-a-repo", "big"), 3<<20)

	cfg := &config.Config{Version: 1, Volume: home, Policy: testutil.PolicyFor(home), Home: home}
	env := guard.Env{Home: home}
	var out, errw bytes.Buffer
	if code := doScanBuilds(cfg, env, &opts{scanBuilds: dev, jsonOut: true, minMB: 1}, now, &out, &errw); code != status.ExitOK {
		t.Fatalf("exit %d: %s", code, errw.String())
	}
	var j struct {
		Repos []repos.Row `json:"repos"`
	}
	if err := json.Unmarshal(out.Bytes(), &j); err != nil {
		t.Fatal(err)
	}
	by := map[string]repos.Row{}
	...
	for _, r := range k.Repos {
		for _, b := range r.Builds {
			if b.Suggest || !strings.Contains(b.Why, "under") && b.Bytes > 0 && b.Bytes < repos.DefaultFloor && r.Name == "idle-rs" {
	...
	out.Reset()
	if code := doScanBuilds(cfg, env, &opts{scanBuilds: dev, verbose: true, minMB: 1}, now, &out, &errw); code != status.ExitOK {
		t.Fatalf("exit %d: %s", code, errw.String())
	}
	s := out.String()
	...
	oldGit := repos.GitOut
	repos.GitOut = func(dir string, args ...string) ([]byte, error) { return nil, errors.New("git: not found") }
	t.Cleanup(func() { repos.GitOut = oldGit })
	out.Reset()
	...
	if code := doScanBuilds(cfg, env, &opts{scanBuilds: filepath.Join(home, "nope")}, now, &out, &errw); code != status.ExitUsage {
